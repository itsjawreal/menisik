...
def _write_fake_repo_root(root: Path, env_text: str = "") -> None:
    """Create a minimal layout that config._looks_like_repo_root accepts."""
...
def _isolated_config_root(env_text: str = ""):
    """Reload config against a fake repo root so the machine's real .env
    cannot leak values into os.environ via setdefault."""
    with tempfile.TemporaryDirectory() as tmp:
        root = Path(tmp)
        _write_fake_repo_root(root, env_text)
...
class EnvFileQuoteTests(unittest.TestCase):
    ...
    def test_quoted_claude_args_in_env_file_parse_as_separate_cli_args(self) -> None:
        ...
        with _isolated_config_root(env_text):
