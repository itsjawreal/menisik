...
def _has_usable_claude_backend() -> bool:
    if not CLAUDE_CMD:
        return False
    return Path(CLAUDE_CMD).exists() or shutil.which(CLAUDE_CMD) is not None or shutil.which("claude") is not None
...
def call_ai(prompt: str, timeout: int = 600, stream_path: Path | None = None) -> str:
    try:
        ...
        if _ACTIVE_BACKEND == "codex" and _has_usable_claude_backend():
            _LOG.warning("Codex backend unavailable (%s). Falling back to Claude for this run.", exc)
            _switch_backend("claude")
            try:
                return _call_backend(prompt, timeout=timeout, stream_path=stream_path)
            except (BackendConfigurationError, BackendRuntimeError) as fallback_exc:
                raise BackendRuntimeError(
                    f"Codex backend unavailable: {exc} | Claude fallback unavailable: {fallback_exc}"
                ) from fallback_exc
        raise
